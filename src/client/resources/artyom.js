...
var Artyom = (function() {
  ...
  Artyom.prototype.execute = function(voz) {
    ...
    if (_this.ArtyomProperties.name && _this.ArtyomProperties.triggered == false) {
      ...
      _this.ArtyomProperties.triggered = true
    }
    _this.debug('>> ' + voz)
    /** @3
    ...
    for (var i = 0; i < _this.ArtyomCommands.length; i++) {
      ...
      for (var c = 0; c < opciones.length; c++) {
        ...
        if (opcion instanceof RegExp) {
          ...
          if (opcion.test(voz)) {
            _this.debug(
              '>> REGEX ' + opcion.toString() + ' MATCHED AGAINST ' + voz + ' WITH INDEX ' + c + ' IN COMMAND ',
              'info'
            )
    ...
    for (var i = 0; i < _this.ArtyomCommands.length; i++) {
      ...
      for (var c = 0; c < opciones.length; c++) {
        ...
        if (voz === opcion) {
          _this.debug(
            '>> MATCHED FULL EXACT OPTION ' + opcion + ' AGAINST ' + voz + ' WITH INDEX ' + c + ' IN COMMAND ',
            'info'
          )
          encontrado = parseInt(c.toString())
          break
    ...
    for (var i = 0; i < _this.ArtyomCommands.length; i++) {
      ...
      for (var c = 0; c < opciones.length; c++) {
        ...
        if (voz.indexOf(opcion) >= 0) {
          _this.debug(
            '>> MATCHED INDEX EXACT OPTION ' + opcion + ' AGAINST ' + voz + ' WITH INDEX ' + c + ' IN COMMAND ',
            'info'
          )
          encontrado = parseInt(c.toString())
          break
    ...
    if (_this.ArtyomProperties.mode == 'normal') {
      onResultProcessor = function(event) {
        ...
        for (var i = event.resultIndex; i < cantidadResultados; ++i) {
          ...
          _this.recognized_speech = identificated
